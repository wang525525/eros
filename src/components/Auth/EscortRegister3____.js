...
function App() {
  ...
  function handleClick(e){
    ...
    if (inputs.ano==='' && escort.ano!=='') {
      inputs.ano=escort.ano;
    }
    ...
    if (inputs.ano!=='' && inputs.mes!=='' && inputs.dia!=='') {
      ...
      Functions.PostAsync("User","setEscort",{user_id:user.user_id,token:user.token,birth_date:inputs.ano+'-'+inputs.mes+'-'+inputs.dia},context,{name:"callbackContinue",funct:callbackContinue})
    }else{
      let modal = {
                status:true,
                title:"Error",
