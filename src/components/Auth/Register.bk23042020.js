...
function App() {
  ...
  */
  ...
  const [dataAjax, setdataAjax]   =   useState(false)
  ...
  function handleClick(){
    ...
    if (!phoneCode) {
      if (!chekingInputs()){ return false;}
      Functions.PostAsync("User","register",inputs,context,{name:"callback",funct:activeBox})
    }else{
  ...
  function chekingInputs(){
    if(!phoneCode && ((inputs.email==='' || inputs.email===undefined) || (inputs.phone==='' || inputs.phone===undefined)) && !checkStorage){
      modal = {
                status:true,
  ...
  /*Activar casilla de código*/
  function activeBox(data){
    setdataAjax(data);
    setVerify(true);
  }
  ...
  function handleClickSendCode(){
    if(inputs.codeVerify===undefined || inputs.codeVerify===''){
      modal = {
                    status:true,
                    title:"Campos incompletos",
                    message:"Para completar el proceso debes indicar el código de verificación enviado a tu celular",
                    ico:{
                          contentColor:"modal-ico-bg-primary",
                          ico:'fas fa-exclamation pl-3',
                        },
      ...
      if (firebase.auth().settings.appVerificationDisabledForTesting === true) {
        inputs.codeVerify  = 123456
      }
      ...
      if (_phone!=='') {
        firebase.auth().signInWithPhoneNumber(_phone, appVerifier)
          .then(function (confirmationResult) {
            ...
            console.log(inputs.codeVerify ,_phone,error,'Error');
        });
      }
    }
  }
          ...
          <div className="row justify-content-center mt-3">
            <div className="col-12 col-sm-4 text-center">
              ...
              <div  className={(inputVerify)?"input-group mb-3":"input-group mb-3 d-none"}>
                ...
                <input  maxLength="6" type="numeric"  onChange={KeyUp}  name="codeVerify" className="form-control" placeholder="******" required/>
              </div>
            </div>
          </div>
          <div id="sign-in-button"></div>
          <div className="row justify-content-md-center mt-1">
            {(!inputVerify)?<div className="col col-sm-6">
              {(!continueRegistration)?<div onClick={handleClick} className="btn btn-primary btn-block btn-lg text-white text-decoration-none" href={Config.ConfigAppUrl+"Auth/Register4"} >
                                        ...
                                        </div>:<Link className="btn btn-primary btn-block btn-lg text-white text-decoration-none" href={Config.ConfigAppUrl+"Auth/Register1"} >Continuar Registro</Link>

              }
              </div>:<div className="col col-sm-6"><div onClick={handleClickSendCode} className="btn btn-primary btn-block btn-lg text-white text-decoration-none">Enviar código de verificación</div></div>
            }
