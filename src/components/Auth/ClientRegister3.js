  ...
  */
  ...
  useEffect(()=>{
    ...
    if (user.token!==undefined) {
        let str     =   user.phone;
        let init    =   store.get("geoip").cod_phone.length;
        let res     =   ""
        if (str!==undefined) {
          str.substr(init,10)
        }
        setInputs({cod_country:store.get("geoip").cod_phone,email:user.email,phone:res,})
    }else {
          ...
          <div className="row justify-content-md-center mt-1">
            <div className="col-12 col-sm-4 text-center">
              {(!continueRegistration)?<div onClick={handleClick} className="btn btn-primary btn-block btn-lg text-white text-decoration-none" href={Config.ConfigAppUrl+"Auth/Register4"} >
                                        ...
                                        </div> :<Link className="btn btn-primary btn-block btn-lg text-white text-decoration-none" href={Config.ConfigAppUrl+"Auth/clientRegister4"} >
                                                  Continuar Registro
                                                </Link>
